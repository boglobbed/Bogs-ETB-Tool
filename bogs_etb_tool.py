...
def find_game_path():
    rel_paths = [
        ...
        r"Program Files (x86)\Steam\steamapps\common\EscapeTheBackrooms",
        r"Program Files\Steam\steamapps\common\EscapeTheBackrooms",
        r"Games\steamapps\common\EscapeTheBackrooms",
        r"Games\Steam\steamapps\common\EscapeTheBackrooms",
        r"Games\SteamLibrary\steamapps\common\EscapeTheBackrooms",
    ]
    candidates = []
    ...
    for c in candidates:
        exe = os.path.join(c, "EscapeTheBackrooms", "Binaries", "Win64", "Backrooms-Win64-Shipping.exe")
...
def save_game_path(p):
    with open(os.path.join(get_exe_dir(), "game_path.txt"), "w") as f:
        f.write(p)

def load_game_path():
    path = os.path.join(get_exe_dir(), "game_path.txt")
    if os.path.exists(path):
        with open(path, "r") as f:
            return f.read().strip()
    return None

GAME_PATH = load_game_path() or find_game_path()
if GAME_PATH and not load_game_path():
...
def get_versions_dir():
    path = os.path.join(get_exe_dir(), "versions")
...
def collect_files(src_dir, skip=("Engine",)):
    files = []
    for item in os.listdir(src_dir):
        ...
        if os.path.isdir(full):
            for root_dir, dirs, fnames in os.walk(full):
                for fname in fnames:
                    abs_src = os.path.join(root_dir, fname)
                    rel = os.path.relpath(abs_src, src_dir)
                    files.append((abs_src, rel))
        else:
            files.append((full, item))
    return files
    ...
    if os.path.isdir(src):
        if os.path.exists(dst):
            for item in os.listdir(src):
                move_item(os.path.join(src, item), os.path.join(dst, item))
...
def fix_steam_manifest():
    import re
    manifest_path = os.path.join(os.path.dirname(GAME_PATH), "..", "appmanifest_1943950.acf")
    ...
    try:
        ...
        content = re.sub(r'"BytesToDownload"\s+"[^"]+"', '"BytesToDownload"\t\t"0"', content)
        content = re.sub(r'"BytesDownloaded"\s+"[^"]+"', '"BytesDownloaded"\t\t"0"', content)
        content = re.sub(r'"BytesToStage"\s+"[^"]+"', '"BytesToStage"\t\t"0"', content)
        content = re.sub(r'"BytesStaged"\s+"[^"]+"', '"BytesStaged"\t\t"0"', content)
...
def do_switch_thread(version_name, backup_name_for_current):
    ...
    try:
        ...
        current_name = current if current else backup_name_for_current
        backup_dir = os.path.join(get_versions_dir(), current_name)
        os.makedirs(backup_dir, exist_ok=True)

        items_to_move_out = [i for i in os.listdir(GAME_PATH) if i != "Engine"]
        items_to_move_in = [i for i in os.listdir(version_path) if i != "Engine"]
        total = len(items_to_move_out) + len(items_to_move_in)

        root.after(0, lambda: progress_bar.config(maximum=max(total, 1), value=0))
        step = [0]

        def tick():
            step[0] += 1
            root.after(0, lambda v=step[0]: progress_bar.config(value=v))

        if items_to_move_out:
        ...
        root.after(0, refresh_versions_now)
        root.after(0, lambda: set_switch_ui_locked(False))
        switching[0] = False

        if auto_launch.get():
            root.after(0, launch_etb)

    except Exception as e:
        root.after(0, lambda err=str(e): version_status.config(text=f"Error: {err}", fg="#e74c3c"))
        root.after(0, lambda: progress_bar.config(value=0))
        root.after(0, lambda: set_switch_ui_locked(False))
        switching[0] = False
...
def is_etb_running():
    ...
    return 'Backrooms-Win64-Shipping.exe' in result.stdout
...
def switch_version():
    ...
    if is_etb_running():
        ...
        if not messagebox.askyesno("ETB is Running", "Escape the Backrooms is still running!\nClose it before switching versions.\n\nContinue anyway?"):
            return
    ...
    backup_name_for_current = None

    if not os.path.exists(os.path.join(get_exe_dir(), "current_version.txt")):
        ...
        tk.Label(popup, text="What version are your current game files?\n(They will be backed up to your versions folder)",
                 bg="#1a1a1a", fg="#aaaaaa", font=("Segoe UI", 9), wraplength=250).pack(pady=(12, 6))
        entry = tk.Entry(popup, font=("Segoe UI", 9), bg="#2a2a2a", fg="#dddddd", insertbackground="white", relief="flat")
        entry.pack(padx=16, fill="x", ipady=4)
        entry.focus()
        ...
        def do_confirm():
            ...
            if name:
                result[0] = name
                popup.destroy()
        entry.bind("<Return>", lambda e: do_confirm())
        tk.Button(popup, text="Confirm", command=do_confirm, bg="#c0392b", fg="white", relief="flat",
                  font=("Segoe UI", 9), cursor="hand2", padx=8, pady=4).pack(pady=8)
        popup.wait_window()

        if not result[0]:
            return
        backup_name_for_current = result[0]

    switching[0] = True
    set_switch_ui_locked(True)
    version_status.config(text="Starting switch...", fg="#aaaaaa")

    t = threading.Thread(target=do_switch_thread, args=(version_name, backup_name_for_current), daemon=True)
    t.start()
...
def edit_keybind():
    ...
    if not bindings or selection[0] >= len(bindings):
        return
    action_name, display, raw_line = bindings[selection[0]]

    import re
    cur_key = re.search(r'Key=(\S+)\)', raw_line)
    cur_key = cur_key.group(1) if cur_key else ""
    ...
    mod_frame_e = tk.Frame(popup, bg="#1a1a1a")
    mod_frame_e.pack(pady=(6, 0))
    shift_var = tk.BooleanVar(value=cur_shift)
    ctrl_var = tk.BooleanVar(value=cur_ctrl)
    alt_var = tk.BooleanVar(value=cur_alt)
    tk.Checkbutton(mod_frame_e, text="Shift", variable=shift_var, bg="#1a1a1a", fg="#aaaaaa", selectcolor="#1a1a1a", activebackground="#1a1a1a", font=("Segoe UI", 8)).pack(side="left", padx=4)
    tk.Checkbutton(mod_frame_e, text="Ctrl", variable=ctrl_var, bg="#1a1a1a", fg="#aaaaaa", selectcolor="#1a1a1a", activebackground="#1a1a1a", font=("Segoe UI", 8)).pack(side="left", padx=4)
    ...
    key_entry.bind("<Return>", lambda e: do_edit())
    tk.Button(popup, text="Save", command=do_edit, bg="#c0392b", fg="white", relief="flat", font=("Segoe UI", 9), cursor="hand2", padx=8, pady=4).pack(pady=8)
...
edit_kb_btn = tk.Button(kb_btn_frame, text="Edit Keybind", command=edit_keybind, bg="#2a2a2a", fg="#dddddd", activebackground="#3a3a3a", activeforeground="white", relief="flat", font=("Segoe UI", 9), cursor="hand2", padx=8, pady=4)
edit_kb_btn.pack(side="left", padx=(0, 6))
